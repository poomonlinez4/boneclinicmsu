...
class ShowListBuyCourse extends StatefulWidget {
  const ShowListBuyCourse({Key? key}) : super(key: key);

  @override
  State<ShowListBuyCourse> createState() => _ShowListBuyCourseState();
}

class _ShowListBuyCourseState extends State<ShowListBuyCourse> {
  bool load = true;
  bool? haveCourse;
  List<HistoryBuycourseModel> historybuycourseModels = [];
  ...
  Future<void> readAPI() async {
    ...
    String urlAPI =
        '${MyConstant.domain}/boneclinic/getBuyCourseWhereIdBuyer.php?isAdd=true&idBuyer=$id_buyer';
    await Dio().get(urlAPI).then((value) {
      ...
      if (value.toString() == 'null') {
        ...
        for (var item in json.decode(value.data)) {
          HistoryBuycourseModel model = HistoryBuycourseModel.fromMap(item);
          ...
          setState(() {
            ...
            historybuycourseModels.add(model);
          });
        }
      }
    });
  ...
  LayoutBuilder listBuyCourse() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) => ListView.builder(
        itemCount: historybuycourseModels.length,
        itemBuilder: (context, index) => GestureDetector(
          onTap: () {
            print('### You Click Index ==>> $index');
            showAlertDialog(historybuycourseModels[index], listImages[index]);
          },
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ShowTitle(
                          title: 'เวลา ${historybuycourseModels[index].time} ',
                          textStyle: MyConstant().h3GreenStyle(),
                        ),
                        ShowTitle(
                          title: historybuycourseModels[index].name_course,
                          textStyle: MyConstant().h3BlueStyle(),
                        ),
                        // ShowTitle(
                        //   title:
                        //       'ราคา  ${historybuycourseModels[index].price} THB',
                        //   textStyle: MyConstant().h3BlueStyle(),
                        // ),
                        // ShowTitle(
                        //   title: cutWord(
                        ...
                        ShowTitle(
                          title:
                              'สิทธิรักษา:${historybuycourseModels[index].quantity} ครั้ง ',
